...
public class PureHuskySort<X extends Comparable<X>> {
    ...
    public void sort(final X[] xs, final String[] strings, Boolean perfect) {
        // NOTE: we start with a random shuffle
        // This is necessary if we might be sorting a pre-sorted array. Otherwise, we usually don't need it.
        if (mayBeSorted) Collections.shuffle(Arrays.asList(xs));
        // NOTE: First pass where we code to longs and sort according to those.
        //final Coding coding = huskyCoder.huskyEncode(xs);
        //final long[] longs = coding.longs;
        introSort(xs, strings, 0, strings.length, 2 * floor_lg(xs.length));
        // NOTE: Second pass (if required) to fix any remaining inversions.
        if (perfect)
            return;
        if (useInsertionSort)
    ...
    public PureHuskySort(){
        this.huskyCoder = (HuskyCoder<X>) HuskyCoderFactory.chineseEncoder;
        this.mayBeSorted = false;
        this.useInsertionSort = false;
    }
    ...
    private void introSort(final X[] objects, final String[] longs, final int from, final int to, final int depthThreshold) {
        // CONSIDER merge with IntroHuskySort
        if (to - from <= sizeThreshold + 1) {
            insertionSort(objects, longs, from, to);
            return;
        ...
        final int lo = from;
        final int hi = to - 1;
        if (longs[hi].compareTo(longs[lo]) < 0) swap(objects, longs, lo, hi);
        int lt = lo + 1, gt = hi - 1;
        int i = lo + 1;
        while (i <= gt) {
            if (longs[i].compareTo(longs[lo]) < 0) swap(objects, longs, lt++, i++);
            else if (longs[hi].compareTo(longs[i]) < 0) swap(objects, longs, i, gt--);
            else i++;
        }
        swap(objects, longs, lo, --lt);
        swap(objects, longs, hi, ++gt);
        introSort(objects, longs, lo, lt, depthThreshold - 1);
        if (longs[lt].compareTo(longs[gt]) < 0) introSort(objects, longs, lt + 1, gt, depthThreshold - 1);
        introSort(objects, longs, gt + 1, hi + 1, depthThreshold - 1);
    }
    ...
    private static <T extends Comparable<T>> void heapSort(final T[] objects, final String[] longs, final int from, final int to) {
        // CONSIDER removing these size checks. They haven't really been tested.
        if (to - from <= sizeThreshold + 1) {
            insertionSort(objects, longs, from, to);
            return;
    ...
    private static <T extends Comparable<T>> void downHeap(final T[] objects, final String[] longs, int i, final int n, final int lo) {
        final String d = longs[lo + i - 1];
        final T od = objects[lo + i - 1];
        int child;
        while (i <= n / 2) {
            child = 2 * i;
            if (child < n && longs[lo + child - 1].compareTo(longs[lo + child]) < 0) child++;
            if (d.compareTo(longs[lo + child - 1]) >= 0) break;
            longs[lo + i - 1] = longs[lo + child - 1];
            objects[lo + i - 1] = objects[lo + child - 1];
            i = child;
        }
    ...
    static <T extends Comparable<T>> void insertionSort(final T[] objects, final String[] longs, final int from, final int to) {
        for (int i = from + 1; i < to; i++)
            if (OPTIMIZED)
                swapIntoSorted(objects, longs, i);
            else
                for (int j = i; j > from && (longs[j].compareTo(longs[j - 1]) < 0); j--)
                    swap(objects, longs, j, j - 1);
    }
    /**
    ...
    private static <T extends Comparable<T>> void swap(final T[] xs, final String[] longs, final int i, final int j) {
        // Swap longs
        final String temp1 = longs[i];
        longs[i] = longs[j];
        longs[j] = temp1;
        // Swap xs
        final T temp2 = xs[i];
     ...
     */
    private static <T extends Comparable<T>> void swapIntoSorted(final T[] xs, final String[] longs, final int i) {
     ...
     */
    static <T extends Comparable<T>> void swapInto(final T[] xs, final String[] longs, final int i, final int j) {
        if (j > i) {
            final T x = xs[j];
            System.arraycopy(xs, i, xs, i + 1, j - i);
            xs[i] = x;
            final String l = longs[j];
